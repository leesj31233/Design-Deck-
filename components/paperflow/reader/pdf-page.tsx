...
export function PdfPage({ page, scale, documentId, pageIndex, annotations, selected, onResolved }: { page: PdfPageHandle; scale: number; documentId: string; pageIndex: number; annotations: Annotation[]; selected?: string; onResolved: (results: ResolvedAnnotation[]) => void }) {
  ...
  }, [annotations, textReady, scale, documentId, pageIndex, onResolved]);
  useEffect(() => {
    if (!textReady) return;
    const handle = () => {
  ...
  return <div className="pf-page-wrap"><div ref={surface} className="pf-pdf-page" data-pdf-page={pageIndex} data-ready={ready && textReady} style={{ width: page.width * scale, height: page.height * scale, "--scale-factor": scale, "--total-scale-factor": scale } as React.CSSProperties}>
    ...
    <div ref={layer} className="textLayer" aria-label={`선택 가능한 원문 ${pageIndex + 1}페이지`}/>
