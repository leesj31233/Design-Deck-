...
export interface CommandItemDef { id: string; label: string; shortcut?: string; icon?: React.ReactNode; keywords?: string[]; onSelect?: () => void; group?: string; disabled?: boolean; }
export function CommandMenu({ open, onOpenChange, items, placeholder = "Search commands…" }: { open: boolean; onOpenChange: (open: boolean) => void; items: CommandItemDef[]; placeholder?: string }) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent aria-describedby={undefined} className="overflow-hidden p-0 sm:w-[600px]"><DialogTitle className="sr-only">Command menu</DialogTitle><Command className="bg-transparent" loop>
